#include "Stopwatch_PrivateTimer.h"
...
/*signed int Timer_P(unsigned int P) {
        ...
        private_timer_Prescaler[TIMER_LOAD] =  (P << 8);
        return TIMER_SUCCESS;
}*/
...
signed int Timer_interrupt( bool set_F) {
        ...
        if (set_F) {
        	private_timer_interrupt[TIMER_INTERRUPT] |=  0x1;
        }
        return private_timer_interrupt[TIMER_INTERRUPT];
}
