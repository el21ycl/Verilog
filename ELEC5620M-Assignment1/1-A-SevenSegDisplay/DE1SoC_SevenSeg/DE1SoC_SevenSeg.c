...
void DE1SoC_SevenSeg_SetSingle(unsigned int display, unsigned int value) {
	...
	DE1SoC_SevenSeg_Write(display,LED[value]);
...
void DE1SoC_SevenSeg_SetDoubleHex(unsigned int display, unsigned int value) {
    ...
    // the left of the specified display.
		    value = value % 16; //The remainder of the division is left as a single digit
		DE1SoC_SevenSeg_SetSingle(display,value);
	    if(value == 0x0){
			Hex = value/16;  //Dividing by an integer leaves the tens digit
    	}
	DE1SoC_SevenSeg_SetSingle(display+1,Hex);
    /** Some examples:
     *
...
void DE1SoC_SevenSeg_SetDoubleDec(unsigned int display, unsigned int value) {
    ...
    // the specified 'display', and the second digit on the display to
    // the left of the specified display.
	           value = value % 10; //The remainder of the division is left as a single digit
	DE1SoC_SevenSeg_SetSingle(display,value);
			if(value == 0x0){
				Dec = value /10; //Dividing by an integer leaves the tens digit
			}
	DE1SoC_SevenSeg_SetSingle(display+1,Dec);
    /** Some examples:
     *
     *	  input | output | HEX(N+1) | HEX(N)
     *    ----- | ------ | -------- | ------
